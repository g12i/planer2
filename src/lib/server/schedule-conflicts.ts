...
export type RoomScheduleEntryRow = {
  entry_id: string;
  start_date_time: string;
  end_date_time: string;
  room_usos_id: string;
  ...
  for (const entry of entries) {
    ...
    for (const other of allYearEntries) {
      ...
      conflicts.push({
        entry_id: entry.id,
        severity: "error",
        message: "Prowadzący jest już zajęty w tym samym terminie.",
      });
      break;
    }
  }
...
function roomDoubleBookingConflicts(
  entries: PlanScheduleEntryRow[],
  groupsById: Map<string, PlanGroupRow>,
  allYearRoomEntries: RoomScheduleEntryRow[],
): ScheduleConflict[] {
  const conflicts: ScheduleConflict[] = [];

  for (const entry of entries) {
    const group = groupsById.get(entry.plan_semester_subject_group_id);
    const roomId = group?.room_usos_id;
    if (!roomId) {
      continue;
    }

    for (const other of allYearRoomEntries) {
      if (other.entry_id === entry.id) {
        continue;
      }
      if (other.room_usos_id !== roomId) {
        continue;
      }
      if (
        !rangesOverlap(
      ...
      conflicts.push({
        entry_id: entry.id,
        severity: "error",
        message: "Sala jest już zajęta w tym samym terminie.",
      });
      break;
    }
  }
...
export function computeScheduleConflicts(input: {
  ...
  conflicts.push(
    ...doubleBookingConflicts(input.entries, groupsById, input.allYearEntries),
  );

  conflicts.push(
    ...roomDoubleBookingConflicts(
      input.entries,
      groupsById,
      input.allYearRoomEntries,
    ),
  );
